...
//type headerRequesterFn func(bc.Hash) error
...
func (f *Fetcher) insert(peer string, block *legacy.Block) {
	...
	log.Debug("Importing propagated block", "peer", peer, "number", block.Height, "hash", hash)
	go func() {
		...
		if err != nil {
			log.Debug("Unknown parent of propagated block", "peer", peer, "number", block.Height, "hash", hash, "parent", block.PreviousBlockHash.String())
			return
		}
		if parent == nil {
			log.Debug("Unknown parent of propagated block", "peer", peer, "number", block.Height, "hash", hash, "parent", block.PreviousBlockHash.String())
		...
		if _, err := f.insertChain(block); err != nil {
			log.Debug("Propagated block import failed", "peer", peer, "number", block.Height, "hash", hash, "err", err)
			return
		}
